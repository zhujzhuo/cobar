...
public class FormatUtil {
	...
	private static final String[] timeFormat = new String[] { "d ", "h ", "m ",
			"s ", "ms" };
	...
	public static final String format(int i, int fillLength) {
		return format(Integer.toString(i), fillLength, defaultSplitChar,
				ALIGN_RIGHT);
	}

	public static final String format(long l, int fillLength) {
		return format(Long.toString(l), fillLength, defaultSplitChar,
				ALIGN_RIGHT);
	}

	public static final String format(String s, int fillLength, char fillChar,
			int align) {
		if (s == null) {
			s = "";
		} else {
	...
	public static String formatStore(long store) {
		if (store > TB) {
			...
			return new StringBuilder(String.valueOf(mem)).append(" ")
					.append("TB").toString();
		} else if (store > GB) {
			...
			return new StringBuilder(String.valueOf(store)).append(" ")
					.append("B").toString();
		}
	}
