 ...
 */
public class PropertyReload extends AbstractController implements
		InitializingBean {
	...
	protected ModelAndView handleRequestInternal(HttpServletRequest request,
		...
		if (logger.isWarnEnabled()) {
			StringBuilder log = new StringBuilder(user.getUsername())
					.append(" | do ").append(types).append(" | cobar:");
			for (int i = 0; i < index.length; i++) {
				log.append(xmlAccesser.getCobarDAO().getCobarById(index[i]))
						.append(" ");
			}
		...
		try {
			...
			case CONFIG_RELOAD:
				for (int i = 0; i < index.length; i++) {
					CobarDO c = xmlAccesser.getCobarDAO()
							.getCobarById(index[i]);
					Map<String, Object> map = new HashMap<String, Object>();
					map.put("name",
							CobarStringUtil.htmlEscapedString(c.getName()));
					if (c.getStatus().equals(ConstantDefine.ACTIVE)) {
						CobarAdapterDAO perf = cobarAccesser
								.getAccesser(index[i]);
