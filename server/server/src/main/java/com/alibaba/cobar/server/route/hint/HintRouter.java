...
public class HintRouter {
	...
	public static void routeFromHint(Object frontConn, SchemaConfig schema,
			RouteResultset rrs, int prefixIndex, final String sql)
		...
		final Pair<String[], Object[][]> partitionOperand = hint
				.getPartitionOperand();
		...
		if (table == null || schema.getTables() == null
			...
			if (dataNodes != null && !dataNodes.isEmpty()) {
				...
				if (replicaIndex != null
					...
					nodes[0] = new RouteResultsetNode(schema.getDataNode(),
							replicaIndex, outputSql);
					logExplicitReplicaSet(frontConn, sql, rrs);
					return;
				}
			}
			nodes[0] = new RouteResultsetNode(schema.getDataNode(), replica,
					outputSql);
		...
		if (dataNodes != null && !dataNodes.isEmpty()) {
			RouteResultsetNode[] nodes = new RouteResultsetNode[dataNodes
					.size()];
			rrs.setNodes(nodes);
			int i = 0;
			...
			for (Pair<Integer, Integer> pair : dataNodes) {
				...
				if (replicaIndex != null
						&& RouteResultsetNode.DEFAULT_REPLICA_INDEX.intValue() != replicaIndex
								.intValue()) {
					replicaSet = true;
					nodes[i] = new RouteResultsetNode(dataNodeName,
							replicaIndex, outputSql);
				} else {
					replicaSet = replicaSet
							|| (replica != RouteResultsetNode.DEFAULT_REPLICA_INDEX);
					nodes[i] = new RouteResultsetNode(dataNodeName, replica,
							outputSql);
				}
				++i;
			}
		...
		if (partitionOperand == null) {
			...
			for (int i = 0; i < nodes.length; ++i) {
				nodes[i] = new RouteResultsetNode(tableDataNodes[i], replica,
						outputSql);
			}
			return;
		}
		...
		if (cols == null || vals == null) {
			throw new SQLSyntaxErrorException(
					"${partitionOperand} is invalid: " + sql);
		}
		...
		Set<String> destDataNodes = calcHintDataNodes(rule, cols, vals,
				tableDataNodes);
		RouteResultsetNode[] nodes = new RouteResultsetNode[destDataNodes
				.size()];
		rrs.setNodes(nodes);
		int i = 0;
		boolean replicaSet = false;
	...
	private static Set<String> calcHintDataNodes(RuleConfig rule,
			String[] cols, Object[][] vals, String[] dataNodes) {
		...
		for (Object[] val : vals) {
			...
			Integer[] dataNodeIndexes = calcDataNodeIndexesByFunction(
					rule.getRuleAlgorithm(), parameter);
	...
	private static void logExplicitReplicaSet(Object frontConn, String sql,
			RouteResultset rrs) {
		if (frontConn != null && LOGGER.isInfoEnabled()) {
			StringBuilder s = new StringBuilder();
			s.append(frontConn).append(
					"Explicit data node replica set from, sql=[");
			s.append(sql).append(']');
			LOGGER.info(s.toString());
		}
	}

	private static Integer[] calcDataNodeIndexesByFunction(
			RuleAlgorithm algorithm, Map<String, Object> parameter) {
		...
		if (calRst instanceof Number) {
			...
			throw new IllegalArgumentException(
					"route err: result of route function is wrong type or null: "
							+ calRst);
		}
