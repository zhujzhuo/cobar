...
public final class CobarHint {
	...
	public void setPartitionOperand(Pair<String[], Object[][]> partitionOperand) {
		...
		if (columns == null) {
			...
			this.partitionOperand = new Pair<String[], Object[][]>(colUp,
					partitionOperand.getValue());
		}
	}
	...
	private void parse(String sql) throws SQLSyntaxErrorException {
		cobarHint: for (;;) {
			...
			String hintName = sql.substring(currentIndex + 1, hintNameEnd)
					.trim();
			...
			if (hintParser != null) {
				...
				throw new SQLSyntaxErrorException("unrecognized hint name: ${"
						+ hintName + "}");
			}
		}
