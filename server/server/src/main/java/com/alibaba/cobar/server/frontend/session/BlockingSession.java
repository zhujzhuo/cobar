 ...
 */
package com.alibaba.cobar.server.frontend.session;
...
import com.alibaba.cobar.server.backend.executor.BlockingMultiNodeExecutor;
...
public class BlockingSession implements Session {
	...
	private final ConcurrentHashMap<RouteResultsetNode, BlockingChannel> target;
	private final BlockingSingleNodeExecutor singleNodeExecutor;
	private final BlockingMultiNodeExecutor multiNodeExecutor;
	private final BlockingCommitExecutor commitExecutor;
	private final BlockingRollbackExecutor rollbackExecutor;

	public BlockingSession(ServerConnection source) {
		this.source = source;
		this.target = new ConcurrentHashMap<RouteResultsetNode, BlockingChannel>();
		this.singleNodeExecutor = new BlockingSingleNodeExecutor();
		this.multiNodeExecutor = new BlockingMultiNodeExecutor();
		this.commitExecutor = new BlockingCommitExecutor();
		this.rollbackExecutor = new BlockingRollbackExecutor();
	}
	...
	public ConcurrentHashMap<RouteResultsetNode, BlockingChannel> getTarget() {
		return target;
	}
	...
	public void cancel(FrontendConnection sponsor) {
		...
		source.writeErrMessage(ErrorCode.ER_QUERY_INTERRUPTED,
	...
	public void terminate() {
		...
		try {
			...
			for (RouteResultsetNode rrn : target.keySet()) {
				BlockingChannel c = target.remove(rrn);
				if (c != null) {
					c.close();
				}
			}
	...
	/**
	 * MUST be called at the end of {@link BlockingNodeExecutor}
	 * 
