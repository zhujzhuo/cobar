 ...
 * @author xianmao.hexm
 */
public final class BlockingRollbackExecutor extends BlockingNodeExecutor {
    private static final Logger LOGGER = Logger.getLogger(BlockingRollbackExecutor.class);
    ...
    public void rollback(final BlockingSession session) {
        ...
        final ConcurrentMap<RouteResultsetNode, BlockingChannel> target = session.getTarget();
    ...
    private void _rollback(BlockingMySQLChannel mc, BlockingSession session) {
        final ServerConnection source = session.getSource();
    ...
    private void handleException(BlockingChannel mc, BlockingSession session, Exception e) {
        isFail.set(true);
        if (decrementCountBy(1)) {
            try {
                session.clear();
