 ...
 */
public final class BlockingMultiNodeExecutor extends BlockingNodeExecutor {
    private static final Logger LOGGER = Logger.getLogger(BlockingMultiNodeExecutor.class);
    ...
    private final BlockingCommitExecutor icExecutor = new BlockingCommitExecutor() {
        @Override
        ...
        protected Logger getLogger() {
            return BlockingMultiNodeExecutor.LOGGER;
        }
    ...
    public void execute(RouteResultsetNode[] nodes, final boolean autocommit, final BlockingSession ss, final int flag) {
        ...
        // 多节点处理
        ConcurrentMap<RouteResultsetNode, BlockingChannel> target = ss.getTarget();
        for (RouteResultsetNode rrn : nodes) {
            BlockingChannel c = target.get(rrn);
            if (c != null) {
                c.setRunning(true);
            }
        }
        ...
        for (final RouteResultsetNode rrn : nodes) {
            final BlockingChannel c = target.get(rrn);
            if (c != null) {
     ...
     * 执行
     */
    private void execute0(RouteResultsetNode rrn, BlockingChannel c, boolean autocommit, BlockingSession ss, int flag) {
        ...
        try {
            // 执行并等待返回
            BinaryPacket bin = ((BlockingMySQLChannel) c).execute(rrn, sc, autocommit);

            // 接收和处理数据
            final ReentrantLock lock = BlockingMultiNodeExecutor.this.lock;
            lock.lock();
            try {
                switch (bin.data[0]) {
                case ErrorPacket.FIELD_COUNT:
                    c.setRunning(false);
                    handleFailure(ss, rrn, new BinaryErrInfo((BlockingMySQLChannel) c, bin, sc, rrn));
                    break;
                ...
                default: // HEADER|FIELDS|FIELD_EOF|ROWS|LAST_EOF
                    final BlockingMySQLChannel mc = (BlockingMySQLChannel) c;
                    if (fieldEOF) {
                        for (;;) {
                            bin = mc.receive();
                            switch (bin.data[0]) {
     ...
     * 处理RowData数据
     */
    private void handleRowData(final RouteResultsetNode rrn, BlockingChannel c, BlockingSession ss) throws IOException {
        final ServerConnection source = ss.getSource();
        BinaryPacket bin = null;
        int size = 0;
        for (;;) {
            bin = ((BlockingMySQLChannel) c).receive();
            switch (bin.data[0]) {
            case ErrorPacket.FIELD_COUNT:
                c.setRunning(false);
                handleFailure(ss, rrn, new BinaryErrInfo(((BlockingMySQLChannel) c), bin, source, rrn));
                return;
            case EOFPacket.FIELD_COUNT:
                c.setRunning(false);
     ...
     * 处理下一个任务
     */
    private void handleNext(final RouteResultsetNode rrn, final BlockingChannel c, final BlockingSession ss) {
        final ServerConnection sc = ss.getSource();
        sc.getProcessor().getExecutor().execute(new Runnable() {
            @Override
            public void run() {
    ...
    protected static class BinaryErrInfo implements ErrInfo {
        ...
        private BlockingMySQLChannel mc;

        public BinaryErrInfo(BlockingMySQLChannel mc, BinaryPacket bin, ServerConnection sc, RouteResultsetNode rrn) {
            this.mc = mc;
            this.source = sc;
            this.rrn = rrn;
            ErrorPacket err = new ErrorPacket();
