 ...
 */
package com.alibaba.cobar.server.route.visitor;
...
public final class PartitionKeyVisitor implements SQLASTVisitor {
	...
	static {
		...
		VERDICT_PASS_THROUGH_WHERE
				.add(ComparisionNullSafeEqualsExpression.class);
	...
	public static boolean isPartitionKeyOperandSingle(Expression expr,
			ASTNode parent) {
		return parent == null && expr instanceof ReplacableExpression
				&& PARTITION_OPERAND_SINGLE.contains(expr.getClass());
	}

	public static boolean isPartitionKeyOperandIn(Expression expr,
			ASTNode parent) {
	...
	private void addColumnValueIndex(String table, String column, Object value,
	...
	private void addIntoColumnValueIndex(
			Map<Object, Set<Pair<Expression, ASTNode>>> valMap, Object value,
			Expression expr, ASTNode parent) {
		...
		Pair<Expression, ASTNode> pair = new Pair<Expression, ASTNode>(expr,
				parent);
		exprSet.add(pair);
	}
	...
	private Map<String, Map<Object, Set<Pair<Expression, ASTNode>>>> ensureColumnValueIndexByTable(
		...
		Map<String, Map<Object, Set<Pair<Expression, ASTNode>>>> colMap = columnValueIndex
				.get(table);
		if (colMap == null) {
			colMap = new HashMap<String, Map<Object, Set<Pair<Expression, ASTNode>>>>(
					2, 1);
	...
	private void addColumnValue(String tableNameUp, String columnNameUp,
	...
	private Map<String, List<Object>> ensureColumnValueByTable(
			String tableNameUp) {
	...
	private List<Object> ensureColumnValueList(
			Map<String, List<Object>> columnValue, String column) {
	...
	// ---temp
	// state------------------------------------------------------------------
	private final Map<Object, Object> evaluationParameter = Collections
			.emptyMap();
	...
	private void visitChild(int idLevel, boolean verdictColumn,
			boolean verdictGroupFunc, ASTNode... nodes) {
	...
	private void visitChild(int idLevel, boolean verdictColumn,
			boolean verdictGroupFunc, List<? extends ASTNode> nodes) {
		...
		if (tr == null) {
			...
			for (Identifier tb : tbs) {
				if (tb instanceof Wildcard) {
					...
					schemaTrimmed = schemaTrimmed
							|| trim == Identifier.PARENT_TRIMED;
					customedSchema = customedSchema
							|| trim == Identifier.PARENT_IGNORED;
				} else {
		...
		if (collist != null) {
			for (int i = 0; i < collist.size(); ++i) {
				...
				if (isRuledColumn(tableName, colName)) {
					List<Object> valueList = ensureColumnValueList(colVals,
							colName);
					Map<Object, Set<Pair<Expression, ASTNode>>> valMap = ensureColumnValueIndexObjMap(
							colValsIndex, colName);
					for (RowExpression row : rows) {
						Expression expr = row.getRowExprList().get(i);
						Object value = expr == null ? null : expr
	...
	public void visit(BetweenAndExpression node) {
		...
		if (verdictColumn && !node.isNot() && fst instanceof Identifier) {
			...
			if (isRuledColumn(table, col.getIdTextUpUnescape())) {
				...
				if (e1 != Expression.UNEVALUATABLE
						&& e2 != Expression.UNEVALUATABLE && e1 != null
						&& e2 != null) {
					if (compareEvaluatedValue(e1, e2)) {
						addColumnValue(table, col.getIdTextUpUnescape(), e1,
								node, null);
					}
				}
			}
		...
		if (verdictColumn && (operand instanceof Identifier)) {
			...
			if (isRuledColumn(table, col.getIdTextUpUnescape())) {
				...
				case ComparisionIsExpression.IS_FALSE:
					addColumnValue(table, col.getIdTextUpUnescape(),
							LiteralBoolean.FALSE, node, null);
					break;
				case ComparisionIsExpression.IS_TRUE:
					addColumnValue(table, col.getIdTextUpUnescape(),
							LiteralBoolean.TRUE, node, null);
					break;
				case ComparisionIsExpression.IS_NULL:
					addColumnValue(table, col.getIdTextUpUnescape(), null,
							node, null);
					break;
				}
			}
		...
		if (verdictColumn) {
			if (left instanceof Identifier) {
				comparisionEquals((Identifier) left,
						right.evaluation(evaluationParameter), false, node);
			} else if (right instanceof Identifier) {
				comparisionEquals((Identifier) right,
						left.evaluation(evaluationParameter), false, node);
			}
		}
	}
	...
	public void visit(ComparisionNullSafeEqualsExpression node) {
		...
		if (verdictColumn) {
			if (left instanceof Identifier) {
				comparisionEquals((Identifier) left,
						right.evaluation(evaluationParameter), true, node);
			} else if (right instanceof Identifier) {
				comparisionEquals((Identifier) right,
						left.evaluation(evaluationParameter), true, node);
			}
		}
	}

	private void comparisionEquals(Identifier col, Object value,
			boolean nullsafe, Expression node) {
		if (value != Expression.UNEVALUATABLE && (nullsafe || value != null)) {
			String table = tableAlias.get(col.getLevelUnescapeUpName(2));
			if (isRuledColumn(table, col.getIdTextUpUnescape())) {
				addColumnValue(table, col.getIdTextUpUnescape(), value, node,
						null);
			}
		}
	}
	...
	public void visit(InExpression node) {
		...
		if (verdictColumn && !node.isNot() && left instanceof Identifier
			...
			if (isRuledColumn(table, colName)) {
				List<Object> valList = ensureColumnValueList(
						ensureColumnValueByTable(table), colName);
				Map<Object, Set<Pair<Expression, ASTNode>>> valMap = ensureColumnValueIndexObjMap(
	...
	public void visit(LogicalAndExpression node) {
		for (int i = 0, len = node.getArity(); i < len; ++i) {
			...
			visitChild(2, verdictColumn && isVerdictPassthroughWhere(oprand),
					false, oprand);
		}
	}
	...
	public void visit(Count node) {
		...
		if (verdictGroupFunc) {
			if (groupFuncType != GROUP_NON && groupFuncType != GROUP_SUM
					|| node.isDistinct()) {
				groupFuncType = GROUP_CANCEL;
			} else {
	...
	public void visit(LikeExpression node) {
		visitChild(2, false, false, node.getFirst(), node.getSecond(),
				node.getThird());
	}
