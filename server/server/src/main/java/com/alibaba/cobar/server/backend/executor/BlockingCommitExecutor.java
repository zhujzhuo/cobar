 ...
 * @author xianmao.hexm
 */
public class BlockingCommitExecutor extends BlockingNodeExecutor {
	private static final Logger LOGGER = Logger
			.getLogger(BlockingCommitExecutor.class);
	 ...
	 * 提交事务
	 */
	public void commit(final OkPacket packet, final BlockingSession session,
			final int initCount) {
		// 初始化
		final ReentrantLock lock = this.lock;
		lock.lock();
		...
		// 执行
		final ConcurrentMap<RouteResultsetNode, BlockingChannel> target = session
				.getTarget();
				...
				try {
					getLogger().error(
							"null is contained in RoutResultsetNodes, source = "
									+ session.getSource() + ", bindChannel = "
									+ target);
				} catch (Exception e) {
				}
				continue;
			}
			final BlockingMySQLChannel mc = (BlockingMySQLChannel) target.get(rrn);
			if (mc != null) {
				mc.setRunning(true);
	...
	private void _commit(BlockingMySQLChannel mc, BlockingSession session) {
				...
				if (decrementCountBy(1)) {
					try {
						if (isFail.get()) { // some other tasks failed
							session.clear();
							source.writeErrMessage(ErrorCode.ER_YES,
						...
						getLogger().warn(
								"exception happens in success notification: "
										+ source, e);
					}
				}
				break;
			case ErrorPacket.FIELD_COUNT:
				...
				if (decrementCountBy(1)) {
					try {
						session.clear();
						getLogger().warn(
								mc.getErrLog(getErrorMessage(),
										mc.getErrMessage(bin), source));
						ByteBuffer buffer = source.allocate();
						source.write(bin.write(buffer, source));
					} catch (Exception e) {
						getLogger().warn(
								"exception happens in failure notification: "
										+ source, e);
					}
				}
				break;
	...
	private void handleException(BlockingChannel mc, BlockingSession session,
			Exception e) {
		isFail.set(true);
		if (decrementCountBy(1)) {
			try {
				...
				getLogger().warn(
						new StringBuilder().append(sc).append(mc)
								.append(getErrorMessage()).toString(), e);
				String msg = e.getMessage();
				sc.writeErrMessage(ErrorCode.ER_YES, msg == null ? e.getClass()
						.getSimpleName() : msg);
			} catch (Exception e2) {
				getLogger().warn(
						"exception happens in failure notification: "
								+ session.getSource(), e2);
			}
		}
	}
