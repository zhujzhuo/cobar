...
public class MultiNodeQueryHandler extends MultiNodeHandler {
    ...
    public MultiNodeQueryHandler(RouteResultsetNode[] route, boolean autocommit, ServerSession session) {
        super(session);
        if (route == null) {
    ...
    public void execute() throws Exception {
        ...
        for (final RouteResultsetNode node : route) {
            ...
            if (conn != null) {
                ...
                CobarModel conf = CobarContainer.getInstance().getConfigModel();
    ...
    @Override
    public void error(byte[] data, MySQLConnection conn) {
    ...
    @Override
    public void ok(byte[] data, MySQLConnection conn) {
    ...
    @Override
    public void fieldEof(byte[] header, List<byte[]> fields, byte[] eof, MySQLConnection conn) {
        lock.lock();
        try {
            if (fieldsReturned) {
                return;
            ...
            for (int i = 0, len = fields.size(); i < len; ++i) {
                ...
                field[3] = ++packetId;
                buffer = ByteBufferUtil.write(field, buffer, source);
            }
            eof[3] = ++packetId;
            buffer = ByteBufferUtil.write(eof, buffer, source);
        } finally {
