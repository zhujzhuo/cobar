...
import com.alibaba.cobar.server.core.defs.Capabilities;
...
public class AuthPacket extends AbstractPacket {
    ...
    public void write(BackendConnection c) {
        ...
        buffer = ByteBufferUtil.write(FILLER, buffer, c);
        if (user == null) {
            buffer = ByteBufferUtil.check(buffer, 1, c);
            buffer.put((byte) 0);
        } else {
            byte[] userData = user.getBytes();
            buffer = ByteBufferUtil.check(buffer, userData.length + 1, c);
            ByteBufferUtil.writeWithNull(buffer, userData);
        }
        if (password == null) {
            buffer = ByteBufferUtil.check(buffer, 1, c);
            buffer.put((byte) 0);
        } else {
            buffer = ByteBufferUtil.check(buffer, ByteBufferUtil.getLength(password), c);
            ByteBufferUtil.writeWithLength(buffer, password);
        }
        if (database == null) {
            buffer = ByteBufferUtil.check(buffer, 1, c);
            buffer.put((byte) 0);
            ...
            buffer = ByteBufferUtil.check(buffer, databaseData.length + 1, c);
            ByteBufferUtil.writeWithNull(buffer, databaseData);
        }
        c.write(buffer);
    }
