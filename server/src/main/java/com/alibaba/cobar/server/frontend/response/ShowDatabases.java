...
public class ShowDatabases {
    ...
    public static void response(ServerConnection c) {
        ...
        Users.User user = CobarContainer.getInstance().getConfigModel().getUsers().getUser(c.getUser());
        if (user != null) {
            TreeSet<String> schemaSet = new TreeSet<String>();
            Set<String> schemas = user.getSchemas();
            if (schemas == null || schemas.size() == 0) {
                schemaSet.addAll(CobarContainer.getInstance().getConfigModel().getSchemas().getSchemas().keySet());
            } else {
